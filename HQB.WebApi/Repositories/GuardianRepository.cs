...
public class GuardianRepository : IGuardianRepository
{
    ...
    public async Task<IEnumerable<Guardian>> GetAllGuardiansAsync()
    {
        ...
        const string sql = "SELECT * FROM Guardian";
    ...
    {
        using var connection = new SqlConnection(_connectionString);
        const string sql = "SELECT * FROM Guardian WHERE ID = @ID";
    ...
    {
        using var connection = new SqlConnection(_connectionString);
        const string sql = "INSERT INTO Guardian (ID, FirstName, LastName, UserID) VALUES (@ID, @FirstName, @LastName, @UserID)";
    ...
    {
        using var connection = new SqlConnection(_connectionString);
        const string sql = "UPDATE Guardian SET FirstName = @FirstName, LastName = @LastName, UserID = @UserID WHERE ID = @ID";
        return await connection.ExecuteAsync(sql, guardian);
    }
    ...
    {
        using var connection = new SqlConnection(_connectionString);
        const string sql = "DELETE FROM Guardian WHERE ID = @ID";
    ...
    {
        using var connection = new SqlConnection(_connectionString);
        const string sql = "SELECT * FROM Guardian WHERE UserID = @UserID";
