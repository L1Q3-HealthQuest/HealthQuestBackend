...
namespace HQB.WebApi.Controllers
{
    ...
    [ApiController]
    public class ArtsController(ZorgAppDbContext context) : ControllerBase
    {
        ...
        // GET: api/Arts/5
        [HttpGet("{id}")]
        public async Task<ActionResult<Arts>> GetArts(int id)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutArts(int id, Arts arts)
        {
            if (id != arts.ID)
            {
                return BadRequest();
        ...
        // DELETE: api/Arts/5
        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteArts(int id)
        {
            var arts = await _context.Artsen.FindAsync(id);
            if (arts == null)
            {
        ...
        private bool ArtsExists(int id)
        {
